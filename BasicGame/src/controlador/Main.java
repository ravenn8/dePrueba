    ...
    * a dins de la classe Rival*/
    ...
    public void simpleUpdate(float tpf) {
        ...
        if(menu.isMenuFinished() && !gameStarted){
            ...
            addDisplay();
            gameStarted = true;
            System.out.println("enemigos"+menu.getNumEnemies());
        }
        
        if(gameStarted){
            camNode.lookAt(car.getSpatial().getWorldTranslation(), Vector3f.UNIT_Y);
            camTrasera.lookAt(car.getSpatial().localToWorld(new Vector3f(0,3,-15), null), Vector3f.UNIT_Y);
            
            camNode.setLocalTranslation(car.getSpatial().localToWorld( new Vector3f( 0, 4, -15), null));
            camTrasera.setLocation(car.getSpatial().localToWorld( new Vector3f( 0, 3, 0), null));
    ...
    private void addDisplay(){        
        ...
        display.addDisplay((int)(settings.getWidth()-(minDimension/2.5f)/2),(int)((minDimension/2.5f)/2),2.5f,(int)(settings.getWidth()-(minDimension/40f)-(minDimension/11.42f)-10),(int)(settings.getHeight()*0.975f),40,(int)(settings.getWidth()-(minDimension/9.23f)-10),(int)(settings.getHeight()*0.95f),9.23f,(int)(settings.getWidth()-(minDimension/11.42f)-10),(int)(settings.getHeight()*0.85f),11.42f,settings.getWidth()/2, (int)(settings.getHeight()*0.88f), 3f);        
        camTrasera = cam.clone();
        camTrasera.setViewPort(0.25f,0.65f ,0.75f,0.95f);
